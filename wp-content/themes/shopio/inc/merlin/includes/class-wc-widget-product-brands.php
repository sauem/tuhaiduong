 ...
 * @extends WC_Widget
 */
class Shopio_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'shopio');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'shopio');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'shopio'),
				'label' => esc_html__('Title', 'shopio'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'shopio'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'shopio'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'shopio'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'shopio'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'shopio'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'shopio'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'shopio'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Shopio_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'shopio');
			...
			echo '<ul class="product-brands" id="shopio-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Shopio_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'shopio'),
		'singular_name'              => esc_html__('Brands', 'shopio'),
		'menu_name'                  => esc_html__('Brands', 'shopio'),
		'all_items'                  => esc_html__('All Brands', 'shopio'),
		'parent_item'                => esc_html__('Parent Brand', 'shopio'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'shopio'),
		'new_item_name'              => esc_html__('New Brand Name', 'shopio'),
		'add_new_item'               => esc_html__('Add New Brands', 'shopio'),
		'edit_item'                  => esc_html__('Edit Brand', 'shopio'),
		'update_item'                => esc_html__('Update Brand', 'shopio'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'shopio'),
		'search_items'               => esc_html__('Search Brands', 'shopio'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'shopio'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'shopio'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
